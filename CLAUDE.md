...
**What this bound does NOT cover.** It is the plugin capture path only — `runtime.evaluate`, and so every hook plus `@akasecurity/scanner`. The web dashboard's `/scan` Server Action (`web-ui/app/(app)/scan/actions.ts` → `packages/local-ops/src/fs-scan.ts`) passes the installed-pack ruleset — pulled and custom packs included — straight to a synchronous in-process `scan()`, with neither the pre-flight nor the deadline. (`aka scan` passes no ruleset and falls back to the bundled registry, so the CLI is not exposed.) Closing that means giving `local-ops` a worker of its own, which is a separate change; until it lands, do not describe the ReDoS bound as covering the whole product.
