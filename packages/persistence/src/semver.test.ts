// TWIN: cli/src/lib/semver.test.ts covers the boundary-forced copy of this
// comparator (`compareSemver`). Keep the two semantically in lockstep — if you
// change the ordering rules or the parse grammar in semver.ts, mirror it in the
// CLI copy and update both suites. The duplication is intentional (persistence
// may not import an app); see the note atop semver.ts.
