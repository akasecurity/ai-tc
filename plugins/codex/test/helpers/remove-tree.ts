...
// meets EPERM on a tree that POSIX would unlink without complaint. That is a
// teardown failing AFTER its test's assertions have all passed, which reads in
// CI as the test itself being broken on Windows.
//
// `@akasecurity/persistence` solves this in its own test harness and
// `@akasecurity/plugin-sdk` carries its own copy, but both sit behind a package
// wall and are not importable here, so this package carries the same rules
// again:
//
