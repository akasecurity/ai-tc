...
import { applyMigrations, reconcileSourceProjectIds, TOKEN_USAGE_COLUMNS } from './migrations.ts';
...
function hasSchemaObject(db: DatabaseSync, type: 'table' | 'index', name: string): boolean {
  return (
...
describe('applyMigrations tag ledger', () => {
  ...
  it('reconciles a RENUMBERED history: adopts present migrations by tag, applies the missing one', () => {
    ...
    try {
      ...
      expect(hasSchemaObject(db, 'table', 'exceptions')).toBe(true);
      expect(appliedTags(db)).toEqual(SQLITE_MIGRATIONS.map((m) => m.tag).sort());
  ...
  it('recreates an index a table-recreate migration drops mid-flight (0005 FK relaxation)', () => {
    ...
    try {
      ...
      applyMigrations(db);
      expect(hasSchemaObject(db, 'index', 'uq_mcp_trust_override')).toBe(true);
  ...
  it('recreates a missing index when adopting an already-applied migration', () => {
    ...
    try {
      ...
      applyMigrations(db);
      expect(hasSchemaObject(db, 'index', 'idx_audit_session_type')).toBe(true);
    } finally {
      db.close();
    }
  });
...
describe('applyMigrations finding resolution', () => {
  it('adds findings.finding_key and creates the finding_resolution table', () => {
    ...
    try {
      ...
      expect(hasSchemaObject(db, 'table', 'finding_resolution')).toBe(true);
    } finally {
      db.close();
    }
  });
...
describe('applyMigrations token-usage backfill', () => {
  ...
  function columnNames(db: DatabaseSync, table: string): Set<string> {
    const rows = db.prepare(`PRAGMA table_xinfo(${table})`).all() as { name: string }[];
  ...
  it('produces exactly the canonical token columns on a fresh store and is idempotent', () => {
    ...
    try {
      applyMigrations(db);
      const afterFirst = columnNames(db, 'audit_events');
      ...
      expect(columnNames(db, 'audit_events')).toEqual(afterFirst);
    } finally {
      db.close();
    }
  });
...
describe('migration 0006 (installed_packs write gate)', () => {
  it('applies cleanly to an alpha.6-era store: gate table seeded closed, trigger + recorded_by present', () => {
    ...
    try {
      applyMigrations(db);
      expect(hasSchemaObject(db, 'table', '_pack_write_gate')).toBe(true);
      expect(
        db
          .prepare(
