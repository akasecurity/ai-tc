 ...
 *
 * Ported from plugins/claude-code's copy. The shape of the check is identical;
 * what differs is this host's hook surface — there is no UserPromptSubmit here,
 * so the end-to-end leg drives `pre-tool-use.js` with Antigravity's camelCase
...
// The scripts that must be in the derived set. Not a substitute for deriving
// it: this is the check that the marker itself has not been renamed away, which
// would otherwise empty the set and make every assertion below vacuous.
//
// This host's set is smaller than Claude Code's, and the absences are the
// documented limits rather than an oversight — so they are worth reading as
// part of the assertion:
//   - no `user-prompt-submit.js`: there is no such event here.
//   - no `pre-invocation.js`: that event carries NO prompt text, so the
//     once-per-conversation pass it hosts has nothing to scan.
//   - no `post-tool-use.js`: that event receives NO tool result, so there is no
//     live response scanning on this host at all.
// If any of those three ever starts building a runtime, this list is what makes
// someone notice that the host contract changed.
const KNOWN_RUNTIME_SCRIPTS = ['pre-tool-use.js', 'filescan.js', 'backfill.js', 'remediate.js'];
...
describe('the built scan worker', () => {
  ...
  it('is found by a real hook’s own resolver, from the built scripts', async () => {
    ...
    const result = spawnSync(process.execPath, [join(SCRIPTS_DIR, 'pre-tool-use.js')], {
      input: JSON.stringify({
        toolCall: { name: 'run_command', args: { CommandLine: `echo ${PULLED_MATCH}` } },
        stepIdx: 0,
        conversationId: 'e2e-worker-conversation',
        workspacePaths: [home],
      }),
      encoding: 'utf8',
    ...
    // Fail-open first, and on THIS host that means an explicit allow literal:
    // silence is read as a deny, so an absent decision would wedge the user's
    // every tool call. Asserting the literal (not the absence of a deny) is
    // what keeps this from passing on empty stdout.
