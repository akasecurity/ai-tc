 ...
 *    whole-value — in this package that is the `maskedMatch` a tool-call
 *    inspection carries (`src/native-host/exchange.ts`), asserted directly in
 *    `native-host/exchange.test.ts`. A generic secret's preview reveals two
 *    characters and cannot fill the window, which is what lets a surface
 *    printing one be searched at all.
 *
 ...
 * `plugins/claude-code/test/helpers/no-echo.ts`,
 * `plugins/codex/test/helpers/no-echo.ts`,
 * `plugins/antigravity/test/helpers/no-echo.ts`,
 * `web-ui/test/helpers/no-echo.ts`,
 * `packages/setup-wizard/test/helpers/no-echo.ts` and
 * `packages/persistence/test/helpers/no-echo.ts` are peers of this file —
