 ...
 *    assertions stay whole-value — `history/scan.test.ts` on `maskedMatch` is
 *    the one here, and it compares against the product's own mask rather than a
 *    literal. A generic secret's preview reveals two characters and cannot fill
 *    the window, which is what lets a surface printing one be searched at all;
 ...
 * `plugins/claude-code/test/helpers/no-echo.ts` are peers of this file — each a
 * copy that takes the `toBeDefined()` guard and a `no-echo.test.ts` with it, or
 * the run length can be widened back with nothing going red.
 */
