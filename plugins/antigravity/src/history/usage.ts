// Token-usage reconciler for Antigravity CLI. Reads a rollout file's usage records
...
// mirrors plugins/claude-code/src/history/usage.ts, but simpler: Antigravity stamps
...
// as the first PreInvocation's can, so its gateway resolution carries the same
// build identity — an attached posture report without it clears the control
// plane's plugin columns. A manifest miss passes through as an explicit
// undefined, which the meta shape tolerates.
...
export async function reconcileSession(
  ...
  const ctx = resolveInventoryContext({
    ...
    // app) from 'codex_cli_rs'/'antigravity-tui'/'codex_exec' (terminal entry
  ...
  const heuristicProvider =
    anchor.model !== undefined ? antigravityProviderFromModelId(anchor.model) : 'unknown';
...
// sources cwd/version straight off the Antigravity usage record (no gitBranch/nwo —
// Antigravity's rollout format carries no per-event git branch field; `session_meta`
...
function buildSessionRoot(
  ...
  // durable per-session fact). Distinguishes a Antigravity session run inside the
