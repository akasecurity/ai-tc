...
// minting a duplicate. Only at-rest (worktree-scan / `aka scan`, kind:
// 'code_change') findings get one; in-flight captures (prompt/response) are
// streamed once and never re-scanned, so there is nothing to correlate against.
//
// Lives here (not in the plugin SDK) so the CLI / web-ui / local-ops — which
// must not import the plugin SDK — can key their `aka scan` findings the exact
// same way the plugin's worktree scanner does, and reconcile cross-tool. The
// plugin SDK re-exports it (finding-key.ts shim), the same pattern fingerprint.ts
// uses for its paired `fingerprintValue`.
//
