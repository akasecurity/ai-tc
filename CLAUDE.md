...
ESLint (`n/no-process-env`) forbids reading `process.env` across the workspace — a violation is a CI failure, not a warning. Eight places in shipped source genuinely need the host environment and opt out — test harnesses that spawn the real hooks — or the real installer scripts — carry inline disables of their own and are out of this table's scope:
...
tools/                repo tooling: the installer one-liners and the suite that drives
                      them end to end, plus the audit-gate workspace package (the CI
                      dependency-audit gate); never shipped
```
