...
describe('scanCoverage', () => {
  ...
  it('pins the two web-chat rows: supported at partial (50) coverage', async () => {
    // The browser extension scans the prompt at the submit gesture on both
    // sites, so neither is "not yet supported" — but it reads no assistant
    // response back out of the DOM, so half of a web chat's two-way traffic is
    // uncovered. That is why these are partial rather than 100, and why they
    // must stay strictly below every terminal harness row rather than matching
    // one: a web-chat row reaching 100 would claim response scanning that the
