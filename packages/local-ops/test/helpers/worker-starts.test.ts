 ...
 * needs a real scan and lives in `../guarded-scan.test.ts`.
 *
 * A peer copy of the plugin SDK's suite, for the same reason the counter beside
 * it is one: a package wall blocks the import, and a copy that arrived without
 * its suite could be weakened back into a constant with every caller green.
 */
