...
// an error — a bearer api key — and is generated rather than written down, so
...
describe('expectNoEchoOf', () => {
  ...
  it('refuses a prefix echo that a whole-value assertion passes', () => {
    const echoed = `refusing to talk to unsafe endpoint with key ${VALUE.slice(0, ECHO_RUN)}...`;
  ...
  it('accepts bytes that carry none of the value', () => {
    ...
    expect(
      refuses('refusing to talk to an unsafe control-plane endpoint: https://host', VALUE),
    ).toBe(false);
  });
