...
// `aka-copilot-ptu-pointer-`) so a directory a failed teardown leaves behind on
// disk still names the case that leaked it.
export function withTempHome<T>(fn: (home: string) => T, prefix = 'aka-copilot-hook-e2e-'): T {
