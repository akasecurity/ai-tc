 ...
 * characters of a bearer key is six raw key bytes, which is a disclosure on
 * its own and a decisive correlator against a key seen elsewhere.
 */
 ...
 *
 * Point this at a value that must not appear AT ALL — the api key material in
 ...
 *
 * This package has no masking surface — it never renders a deliberately
 * truncated preview of a secret — so, like `packages/persistence`'s copy,
 * there is no masked-preview case in this file's own suite.
 */
