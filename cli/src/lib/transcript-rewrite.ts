 ...
 * last place to run a second, subtly different containment rule — so these are
 * transcribed rather than re-invented, and the two must stay in step until the
 * helpers move into a package both surfaces already depend on.
 *
 * IO is node:fs only: no store access, no network, no environment.
 */
...
export async function rewriteContainedFile(
  ...
  const tmpPath = `${realPath}.aka-prune.tmp`;
  try {
    ...
    writeFileSync(tmpPath, result.text, { mode: before.mode & 0o777 });
