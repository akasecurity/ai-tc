...
const MANIFEST_PATH = join(PLUGIN_ROOT, 'plugin.json');

// The emit form of build-info.ts's manifest URL — quote-style tolerant, since
// that is the printer's choice, not ours. Anchored on the whole constructor
// call: this host's specifier is short enough to appear incidentally.
const MANIFEST_URL_RE = /new URL\((['"])(\.\.\/plugin\.json)\1, import\.meta\.url\)/;
...
describe('the emitted scripts resolve the manifest from where they actually sit', () => {
  it('the session-bearing hook carries the manifest URL (the positive control)', () => {
    // Without this, a renamed or re-shaped emit empties the carrier set and
    // the resolution case below passes over nothing. This host has no
    // session-start entry — the inventory pass rides the first PreInvocation.
    expect(emittedCarriers()).toContain('pre-invocation.js');
  });

  it('every emitted carrier resolves the URL to the real manifest', () => {
    for (const name of emittedCarriers()) {
      const resolved = new URL('../plugin.json', pathToFileURL(join(SCRIPTS_DIR, name)));
