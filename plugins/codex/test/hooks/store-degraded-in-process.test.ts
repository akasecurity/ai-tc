  ...
  describe('when the store opens', () => {
    ...
    it('pre-tool-use takes the gateway and commits the capture', async () => {
      ...
      stdin.payload = JSON.stringify({
        ...
        hook_event_name: 'PreToolUse',
        tool_name: 'Bash',
        tool_input: { command: `echo ${PLANTED_SECRET}` },
      });

      await import('../../src/hooks/pre-tool-use.ts');

      // A healthy store records a Bash command ONLY when a rule matched it —
      ...
      // Under the default log-only enforcement a flagged command yields no
      // stdout payload at all — nothing to deny, redact, or warn through — so
