...
import { Badge, cn, SeverityBadge } from '@akasecurity/ui-kit';
...
function EventRow({ event, first, last }: { event: AuditEvent; first: boolean; last: boolean }) {
  ...
  return (
    <div className="grid grid-cols-[52px_24px_1fr] gap-x-3">
      <div className="whitespace-nowrap pt-0.5 text-right font-mono text-label text-text-3">
        {event.time}
      </div>
      <div className="relative flex justify-center">
        ...
        <span
          className={cn(
            'relative z-10 grid size-6 place-items-center rounded-[7px] ring-[3px] ring-surface',
            meta.fill,
            meta.text,
          )}
        >
          {pulse && (
            <span className="absolute inset-0 animate-ping rounded-[7px] bg-current opacity-30" />
          )}
      ...
      <div className={cn('pt-0.5', last ? 'pb-0' : 'pb-4.5')}>
        <div className="flex flex-wrap items-center gap-2">
          ...
          {event.flagged && (
            <Badge variant="critical" className="h-5.5 gap-1">
              <AlertIcon aria-hidden focusable={false} className="size-3" />
              Flagged
            </Badge>
          )}
          {event.internal && (
            <Badge variant="primary" className="h-5.5 gap-1">
        ...
        <div className={cn('mt-0.5 text-xs text-text-2', mono && 'font-mono')}>
          ...
          {linkLabel && (
            <button
              type="button"
              className="ml-2 inline-flex items-center gap-1 align-baseline text-xs font-semibold text-primary hover:underline"
            >
              Open in {linkLabel}
              <ArrowUpRightIcon aria-hidden focusable={false} className="size-3" />
            </button>
          )}
        </div>
      </div>
