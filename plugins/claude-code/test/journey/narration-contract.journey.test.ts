...
describeShimmed(
  'finding narration grounds end-to-end over the real calibration preview and remediation decision composition paths',
  () => {
...
describeShimmed(
  'fail-open leg: the narration signal is absent — both the calibration preview and the remediation decision degrade to the unmodified static frame',
  () => {
    let journey: SetupJourney;
    let preview: { stdout: string; status: number };
    let present: { stdout: string; status: number };
