 ...
 *    whole-value. A generic secret's preview reveals two characters and cannot
 *    fill the window, which is what lets a surface printing one be searched at
 *    all — but the email branch reveals the WHOLE DOMAIN and fills it on
 *    purpose, so a pii/email preview is out of scope for this rather than a
 *    leak it found. Both branches are pinned in this file's own suite.
 *
 * Shared by this package's suites because they sit behind one package wall.
 * Across a wall it cannot be imported, so the copies under `cli/test/helpers/`,
 * `plugins/{claude-code,codex,antigravity}/test/helpers/`, `web-ui/test/helpers/`,
 * `packages/setup-wizard/test/helpers/`, `packages/persistence/test/helpers/` and
 * `packages/remote/test/helpers/` are peers of this file — each a copy that takes
 * the `toBeDefined()` guard and a `no-echo.test.ts` with it, or the run length can
