...
describe('frameCalibration', () => {
  ...
  it('templates the headline copy exactly over the preview values', () => {
    ...
    expect(copy).toBe(
      "I went through Antigravity's recent work — 161 detections, 3 results worth a look. (live keys)",
    );
  });
  ...
  it('uses the singular "detection"/"result" (no trailing s) when the total and important count are each exactly one', () => {
    ...
    expect(copy).toBe(
      "I went through Antigravity's recent work — 1 detection, 1 result worth a look. (live keys)",
    );
  });
  ...
  it('templates over a different preview — the numbers follow the input', () => {
    ...
    expect(copy).toBe(
      "I went through Antigravity's recent work — 10 detections, 2 results worth a look. (live keys)",
    );
  ...
  it('omits the kind parenthetical entirely when nothing surfaced (all suppressed)', () => {
    ...
    expect(copy).toBe(
      "I went through Antigravity's recent work — 10 detections, 0 results worth a look.",
    );
  ...
  it("derives the 'worth a look (…)' kind from the surfaced categories, not a fixed label", () => {
    ...
    expect(copy).toBe(
      "I went through Antigravity's recent work — 24 detections, 4 results worth a look. (personal data)",
    );
...
describe('frameEmptyState', () => {
  ...
  const SCAN_CLEAN_HEADLINE =
    "I looked over Antigravity's recent work — nothing needs your attention right now. You're starting clean; here's what I'd recommend:";
  const NO_HISTORY_HEADLINE =
    "Nothing to learn from yet — Antigravity hasn't left any work on this machine. I'll start each detection category at a careful default:";
  ...
  it('the two empty-state copies are distinct and each states why it is empty', () => {
    ...
    expect(clean).toContain("I looked over Antigravity's recent work");
    // no-history states there is nothing on this machine to learn from.
    expect(noHistory).toContain("Antigravity hasn't left any work on this machine");
  });
