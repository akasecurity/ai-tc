...
export function snapshotStore(db: DatabaseSync, backup: string): void {
  ...
  try {
    ...
    // 0600. This runs BEFORE the rename, and is the only chmod: the rename
    // carries the inode and its mode across, and the one window in which this
    // file can outlive the call is a kill, where no catch runs — tightening
    // after the rename would leave that leftover world-readable.
    tightenFile(partial);
 ...
 *
 * `force` covers only a path that is already gone. A store that cannot be
 * cleared at all (a live handle on Windows) still throws, and then the reset has
 * NOT happened: the original is intact and the next open repeats the whole
 * sequence, which makes the snapshot just taken redundant. Dropping it is what
 * keeps a failing reset from orphaning a full-size copy of the store on every
 * attempt — a bare rename left nothing behind when it failed, and this has to
 * match that.
 */
