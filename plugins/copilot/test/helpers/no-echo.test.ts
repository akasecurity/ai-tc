...
describe('expectNoEchoOf', () => {
  ...
  it('is NOT safe over an email preview, which reveals the whole domain', () => {
    // The boundary the paragraph above draws, written down rather than
    // re-derived: `maskMatch`'s pii/email branch keeps the first local
    // character and the entire domain, which is a run far past the window. A
    // surface printing one of those is out of scope for this helper — and a
    // suite that bound it there would read as a leak it found.
    const email = 'u\u0073er@example-domain-that-is-long.test';
    const preview = maskMatch(email);
    expect(preview).not.toBe(email);
