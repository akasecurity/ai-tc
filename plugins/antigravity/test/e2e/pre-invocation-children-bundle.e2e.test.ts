 ...
 *
 * Antigravity has no SessionStart event, so `pre-invocation.ts` calls
 * `handleSessionStart` itself, once per session (see src/hooks/pre-invocation.ts).
 ...
 * `scripts/pre-invocation.js`, so a child tsup does not emit into `scripts/` is
  ...
  it.each(CHILD_SCRIPTS)('declares %s as a tsup entry', (name) => {
    ...
    expect(
      ...
      `tsup declares no \`${entryKey(name)}\` entry, so scripts/${name} is never emitted and ` +
        'the trigger that resolves it, called from pre-invocation.js, spawns a path that does ' +
        'not exist',
    ).toContain(entryKey(name));
  });

  it.each(CHILD_SCRIPTS)(
    'lands beside pre-invocation.js, since resolution is by sibling',
    (name) => {
      ...
      // rather than a convention. Antigravity has no session-start.js — the
      // sibling here is the hook that actually calls handleSessionStart.
      expect(existsSync(built('pre-invocation.js'))).toBe(true);
      expect(dirname(built(name))).toBe(dirname(built('pre-invocation.js')));
    },
  );

  it.each(CHILD_SCRIPTS)(
    'runs %s to a clean exit, from beside pre-invocation.js, on a machine that is not attached',
    (name) => {
      const home = mkdtempSync(join(tmpdir(), 'aka-agy-pre-invocation-child-'));
      try {
