...
//
// A PEER COPY of the sibling plugins' suite, not an import: a package wall
// blocks the import, and `store-health.ts` is itself a peer copy, so a copy
// without its suite is a module nothing here checks. The one Copilot-specific
// value is the config's `provider`, which on this host is `'unknown'` — see
// plugin-sdk's provider-copilot.ts for why it is never anything else.
