 ...
 * ~/.gemini/antigravity/brain) for secrets that
...
export interface BackfillDeps {
  ...
  // never re-ingests the wizard's own in-progress session). Antigravity exposes no
...
export async function runBackfill(deps: BackfillDeps): Promise<void> {
  ...
  try {
    ...
    if (triage) {
      ...
      const scope = `Scanned ${String(summary.scanned)} messages from the last ${String(summary.windowDays)} days of Antigravity CLI history.`;
      const result =
        summary.findings > 0
