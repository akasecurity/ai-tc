/**
 * The ephemeral setup-wizard judge runner (Antigravity CLI).
 *
 ...
 * scannable rollout store (~/.gemini/sessions), the judgment runs as a
 * SEPARATE, transient `antigravity exec` subprocess that persists NO session file:
 *
 *   antigravity exec --ephemeral --skip-git-repo-check --output-last-message <file> -
 *
 * --ephemeral ("Run without persisting session files to disk", per the Antigravity
 * CLI itself) is REQUIRED, not cosmetic: AKA's own backfill scans
 * ~/.gemini/sessions/**\/rollout-*.jsonl for leaked secrets, so a persisted
 ...
 * argument tells `antigravity exec` to read the prompt from stdin), not argv: argv
 ...
 * under os.tmpdir, removed in a finally), never from stdout — `antigravity exec`
 ...
 *
 * The subprocess inherits the host environment untouched — `antigravity` must be
 * found on PATH, and auth (GEMINI_HOME / ~/.gemini credentials) must survive.
 */
...
export function parseVerdict(lastMessage: string): TriageRecommendation {
  ...
  if (lastMessage.trim() === '') {
    throw new Error('antigravity exec returned an empty last message');
  }
  try {
    return parseRecommendation(lastMessage);
  } catch {
    throw new Error('antigravity exec returned an unparseable TriageRecommendation');
  }
}
...
// untouched. `antigravity` must resolve on PATH and its auth (GEMINI_HOME /
// ~/.gemini credentials) must survive; session persistence is suppressed by
...
// failing the spawn with E2BIG; stdin has no such limit. BOTH of `antigravity exec`'s
...
export interface JudgeDeps {
  // Injected spawn seam: receives the argv AFTER `antigravity`, the subprocess env,
...
// prompt out of ~/.gemini/sessions. Always removes the temp output dir.
...
export function runJudge(hits: readonly TriageHit[], deps: JudgeDeps): TriageRecommendation {
  ...
  // `antigravity`. First statement in the function, so it throws before the rubric
  ...
  try {
    ...
    try {
      ...
      throw new Error(`antigravity exec judge subprocess failed (${spawnFailureMeta(err)})`);
    }
    ...
    try {
      ...
      throw new Error('antigravity exec wrote no last-message file');
    }
