...
describe('SqliteHistorySyncRepository — what is pending', () => {
  it('offers sessions oldest first', () => {
    ...
    expect(db.historySync.pendingSessions(10)).toEqual(['s-early', 's-late']);
  });
  ...
  it('never offers a capture row', () => {
    ...
    const ids = db.historySync.pendingRows('s-1', 100).map((r) => r.id);
  ...
  it('puts the session root before its leaves, whatever their times', () => {
    ...
    expect(db.historySync.pendingRows('s-1', 10)[0]?.id).toBe('s-1');
  });
  ...
  it('still offers a session once only its root has been sent', () => {
    ...
    expect(db.historySync.pendingSessions(10)).toEqual(['s-1']);
    expect(db.historySync.pendingRows('s-1', 10).map((r) => r.id)).toEqual(['s-1-llm', 's-1-tool']);
  });
  ...
  it('offers nothing once everything structural has been sent', () => {
    ...
    expect(db.historySync.pendingSessions(10)).toEqual([]);
  });
  ...
  it('honours the limit', () => {
    ...
    expect(db.historySync.pendingSessions(1)).toHaveLength(1);
    expect(db.historySync.pendingRows('s-1', 2)).toHaveLength(2);
  });
});
...
describe('SqliteHistorySyncRepository — counting', () => {
  it('counts only structural rows, split by what happened to them', () => {
    ...
    expect(db.historySync.counts()).toEqual({ pending: 1, sent: 1, skipped: 1 });
  });

  it('counts an empty store as nothing rather than throwing', () => {
    const db = store.open();
    expect(db.historySync.counts()).toEqual({ pending: 0, sent: 0, skipped: 0 });
  });
  ...
  it('stamps nothing when given no ids', () => {
    ...
    expect(db.historySync.counts().pending).toBe(3);
  });
});
...
describe('SqliteHistorySyncRepository — which deployment the stamps are for', () => {
  it('has no fingerprint until one is recorded', () => {
    const db = store.open();
    expect(db.historySync.endpointFingerprint()).toBeUndefined();
  });
  ...
  it('re-arms delivered rows when the deployment changes', () => {
    const db = store.open();
    seedSession(db, 's-1', 0);
    db.historySync.rearmFor('fingerprint-a');
    db.historySync.markSynced(['s-1', 's-1-llm', 's-1-tool'], T0);
    expect(db.historySync.counts()).toMatchObject({ pending: 0, sent: 3 });

    db.historySync.rearmFor('fingerprint-b');

    expect(db.historySync.endpointFingerprint()).toBe('fingerprint-b');
    expect(db.historySync.counts()).toMatchObject({ pending: 3, sent: 0 });
  });
  ...
  it('leaves permanently skipped rows skipped across a change of deployment', () => {
    ...
    expect(db.historySync.counts().skipped).toBe(1);
  });
});
