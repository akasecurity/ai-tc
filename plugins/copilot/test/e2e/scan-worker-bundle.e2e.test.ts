 ...
 *
 * A peer copy of the three siblings' suite. What differs is this host's own
 * shape: the hook driven end to end is `pre-tool-use.js` with a CLI-dialect
 * payload and the event name on argv, and the fail-open literal asserted is the
 * explicit `permissionDecision` allow — silence would be a deny here.
 */
...
// derived list rather than one somebody has to remember to extend.
const RUNTIME_MARKER = 'isolated scanning is off for ';

/**
 * The scripts that must be in the derived set.
 *
 * Not a substitute for deriving it: this is the check that the marker itself
 * has not been renamed away, which would otherwise empty the set and make every
 * assertion below vacuous.
 *
 * `session-start.js` is deliberately absent and is the documented limit rather
 * than an oversight: that event has no text to scan, so it builds no runtime.
 * If it ever starts carrying the marker, this list is what makes someone notice
 * the hook's job changed.
 */
const KNOWN_RUNTIME_SCRIPTS = ['pre-tool-use.js', 'post-tool-use.js', 'user-prompt-submit.js'];
...
describe('the built scan worker', () => {
  ...
  it('runs and scans with no node_modules anywhere above it', async () => {
    ...
    const dir = tempDir('aka-copilot-scan-worker-');
  ...
  it('is found by a real hook’s own resolver, from the built scripts', async () => {
    ...
    const home = tempDir('aka-copilot-worker-home-');
    ...
    const result = spawnSync(
      process.execPath,
      [join(SCRIPTS_DIR, 'pre-tool-use.js'), 'preToolUse'],
      {
        input: JSON.stringify({
          sessionId: 'e2e-worker-session',
          cwd: home,
          toolName: 'bash',
          toolArgs: { command: `echo ${PULLED_MATCH}`, description: 'echo the marker' },
        }),
        encoding: 'utf8',
        // process.execPath is an absolute node path, so the child needs no host
        // PATH; the home is pointed at the throwaway, which is the only way to
        // redirect ~/.aka (the scripts must not, and do not, hard-resolve it).
        env: { HOME: home, USERPROFILE: home },
        maxBuffer: 64 * 1024 * 1024,
      },
    );

    // Fail-open first, and on THIS event that means an explicit allow literal:
    // the CLI reads a crash as a deny and its reading of silence is unobserved,
    // so an absent decision could wedge the user's every tool call. Asserting
    // the literal (not the absence of a deny) is what keeps this from passing
    // on empty stdout.
    ...
    try {
      // Part one: the rule was measured. Only a rule the runtime treats as
      // UNVERIFIED reaches the timing gate, and the gate runs in a worker of
      // its own — so this row cannot exist unless the resolver found the script
      // from inside the built hook.
