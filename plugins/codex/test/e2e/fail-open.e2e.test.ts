...
const ENFORCING_HOOKS: readonly EnforcingHook[] = [
  ...
  },
  {
    ...
    //
    // So redact and vault emit a systemMessage here rather than a deny. Read it
    // as the setting's consequence rather than as this hook going quiet: with
    // `redactFallback: 'block'` all three enforcing rows read a deny again,
    // which pre-tool-use-decision.test.ts drives directly at both settings. The
    // apply_patch field is rewritable and keeps true redaction either way.
    emits: {
