...
//
// Identical to plugins/claude-code/src/protocol/notes.ts (a package wall blocks
// importing it, same reason exception-guidance.ts is a peer copy): the
// messaging is host-agnostic. `standingBrief` is what session-start.ts wires up
// — this plugin has no vault wiring of its own (see pre-tool-use-decision.ts),
// so `eventNote` has no caller here today, but a `[[aka:<category>:...]]`
// pointer minted by another AKA surface on the same machine (the Claude Code
// plugin's vault, the setup wizard's history scrub) can still surface in this
// host's context — read from a file, a git diff, prior session history — so
// the model needs the SAME standing explanation of what one is, even though
// this host never mints one itself.
