/**
 * What the worktree scanner's file discovery costs, by tree shape.
 *
 * `walkTree()` is the one interpretation of SKIP_DIRS + `.akaignore` that every
 * file-discovery walk in this package goes through — `walkSourceFiles` and the
 * manifest walk alike — so a cost here is paid by every scan the plugin runs.
 * It is measured with `trackGitignore: true`, which is what `walkSourceFiles`
 * asks for: that mode is the one that accumulates a `.gitignore` layer per
 * directory, and the layered evaluation is what these rows track.
 *
 * TWO stacks descend in that mode, not one — `.gitignore` (mark) and
 * `.akaignore` (skip) — so every entry pays the layered evaluation twice.
 *
 ...
 * `test/performance/walk-tree.test.ts`.
 *
 ...
 *     `warmup` then `run` — and cached across both.
...
const drain = (root: string): number => {
  const walked = walkTree(root, { trackGitignore: true });
  let n = 0;
...
describe('a worktree whose bulk is gitignored', () => {
  // `.gitignore` MARKS here rather than skipping — a gitignored file is exactly
  // where a real secret hides — so unlike the inventory walk, BOTH arrangements
  // are fully traversed. 100k rather than 500k: the shape is what the row is
  // for, and the build dominates the measurement well before that.
  row(
...
describe('many .gitignore files', () => {
  ...
  // the offset representation was ported for.
