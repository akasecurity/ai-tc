...
// here (the canonical home of computeFindingKey) and in the plugin-sdk shim's
// test, so any independently replicated derivation MUST produce byte-identical
// output. If two implementations diverge, a finding_key minted by one side
// never reconciles with the other; this pinned digest turns silent drift into a
// test failure. Never update this vector without updating every replica.
...
describe('computeFindingKey', () => {
  ...
  it('differs when the file path differs — two distinct files stay distinct', () => {
